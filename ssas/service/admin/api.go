...
func updateSystem(w http.ResponseWriter, r *http.Request) {
	...
	for k, val := range v {
		...
		if !updateAllowed {
			...
			ssas.OperationFailed(systemEvent)
			service.JsonError(w, http.StatusBadRequest, fmt.Sprintf("attribute: %v is not valid", k), "")
			return
		}
		if !blankAllowed && val == "" {
			systemEvent.Help = fmt.Sprintf("error in request to update group; %v may not be empty", k)
			ssas.OperationFailed(systemEvent)
			service.JsonError(w, http.StatusBadRequest, fmt.Sprintf("attribute: %v may not be empty", k), "")
			return
		}
	}
...
func createV2System(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		service.JsonError(w, http.StatusBadRequest, fmt.Sprintf("could not create v2 system; %s", err), "")
		return
	}
